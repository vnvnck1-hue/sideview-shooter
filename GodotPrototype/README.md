...
| ✎ 대화 UI 랩 | `scenes/DialogueLab.tscn`(`scripts/dialogue_lab.gd`, `main.gd` 상속). 실제 방·인물 위에서 **대사 표시 방식 5종**을 **1~5**로 바꿔 가며 비교한다. **Tab** 다음 방식 · **[ ]** 상대 바꾸기 · **R** 대화 다시 · **0** 플래그 초기화 |
...
| F7 | 대사 표시 방식 순환 (다음 대사부터). 나란히 비교하려면 로비의 "대화 UI 랩" |
| F1 | 로비로 돌아가기 |
| F11 | 전체화면 토글 |
...
| 갈래 | 내용 |
|---|---|
| 표시 방식 | **5종을 표(`DialogueBubble.STYLES`)로 들고 실행 중에 바꾼다** — 1 카타나 제로(머리 위 + 꼬리) · 2 비주얼 노벨(하단 전폭 띠) · 3 자막(상자 없음) · 4 누적 로그(오른쪽에 쌓임, 번호 선택지) · 5 레트로 박스(흰 테두리 + 고정폭). **타자 엔진은 하나**이고 방식은 위치·판·이름 표시·선택지 모양만 정한다. 고른 값은 static 이라 씬을 다시 로드해도 남는다(`DIALOGUE_STYLE` 환경 변수로 고정 가능). 비교는 로비의 **대화 UI 랩** |
...
| `scripts/dialogue_lab.gd` / `scenes/DialogueLab.tscn` | 대화 UI 랩 — 실제 방·인물 위에서 표시 방식 5종을 1~5 로 바꿔 가며 비교 |
