...
| ◻ 공간 테스트 — 이어진 대공간 | `scenes/SpaceLab.tscn`(`scripts/space_lab.gd`, **`main.gd` 상속**). 기본 배경 타일(workshop)로 세운 **20,480 × 1,152px 짜리 한 줄** — 방 5 + 통로 4. 이어지는 방식 두 가지를 나란히 둔다: **그냥 뚫린 통로**(처음부터 양옆 방이 같이 보인다)와 **구역 문**(닫혀 있으면 그 너머가 전혀 안 보이다가, **W/↑** 로 열면 페이드 없이 저쪽 구역이 이쪽과 한 화면에 이어진다 — 방을 갈아 끼우지 않아 지나온 방이 사라지지 않는다). 몬스터는 **30초 웨이브**(4~7마리, 구역 동시 상한 10). **[ ]** 구획 건너뛰기 · 시작 줌 ×2. 방 데이터는 `RoomData.extra` 에 얹어 본 맵을 건드리지 않는다. [공간 테스트 씬](../Docs/SPACE_TEST_SCENE.md) |
