...
##                    포탑 한계(±75°) 밖을 겨누면 **몸이 돌아서고**, 조준 각을 따라 몸통도 젖혀진다.
...
const PRESETS := {
	...
	KEY_1: {"name": "살금살금 (한 발씩 · 낮게)", "speed": 90.0, "ride": 130.0, "stride": 1.20,
	...
	KEY_2: {"name": "정찰 (발을 높이 드는 걸음)", "speed": 150.0, "ride": 112.0, "stride": 1.02,
	...
	KEY_3: {"name": "순찰 (표준 대각보)", "speed": 260.0, "ride": 180.0, "stride": 1.30,
	...
	KEY_4: {"name": "돌격 (빠른 대각 트롯 · 현재 디폴트)", "speed": 430.0, "ride": 135.0, "stride": 1.18,
	...
	KEY_6: {"name": "거미 (한 발씩 사각사각)", "speed": 340.0, "ride": 120.0, "stride": 1.30,
	...
	KEY_5: {"name": "버티기 (사격 자세·넓게)", "speed": 70.0, "ride": 145.0, "stride": 1.55,
...
const TUNE_KEYS := {
	...
	KEY_R: ["ride", 6.0, 90.0, 300.0, "몸 높이"], KEY_V: ["ride", -6.0, 90.0, 300.0, "몸 높이"],
...
func _physics_process(delta: float) -> void:
	...
	_walker.tick(delta)
	_tick_fx(delta)
	_cam.position = _walker.body_pos + Vector2(_walker.speed * 0.18, -40.0)
	if _msg_t > 0.0:
		_msg_t -= delta
	_fx.queue_redraw()
	_debug.queue_redraw()
...
## 포탑이 쐈다. 탄도는 지면에 닿을 때까지 한 줄로 밀어 보고, 예광과 탄착 불꽃만 남긴다.
## (본편에서는 이 자리에 Main 의 사격 경로 — Bullet · RoomSolid.clip_ray · MuzzleBlast 가 들어간다)
...
func _build_hud() -> void:
	...
	keys.text = "마우스 조준 · 좌클릭/J 사격 · A D 걷기 · Shift 달리기 · Space 점프 · 우클릭 드래그로 몸체 잡아끌기   ·   F2 디버그   ·   1~6 걸음새   ·   Q/Z 속도  E/C 다리 벌림  R/V 몸 높이  T/B 보폭  Y/N 발 드는 높이  U/M 스텝 시간  I/, 예측  O/. 접지 유지  P// 동시에 뜨는 발(1=거미·2=트롯)  [/] 기울기  ;/' 조준 젖힘   ·   F5 초기화  F9 수치 출력  ·  F1 로비"
