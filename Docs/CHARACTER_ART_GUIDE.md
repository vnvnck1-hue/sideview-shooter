...
- 자산당 8~16색을 목표로 한다. 피부, 머리, 옷, 외곽선 각각 2~3단계 명암을 기본으로 하고, 강조색은 계기판·배지·시료통 같은 한 곳에만 둔다.
...
4. 1600×900 게임 화면의 zoom 0.5에서도 직업 표식이 남는지 확인한다. 확대 그림에서만 읽히는 눈썹·글자·작은 버튼은 제거한다.
5. 최종 PNG는 80×80 셀, 공통 피벗, 알파 0/255, Native4 픽셀 규격을 검사한 뒤 `Assets/GameReady/Native4/character/npc/<id>/`에 넣는다.
