...
이후 사용자가 첨부한 이미지의 해시가 A 원화와 일치했고, 현재 외관을 마음에 들어 하며 원화 보존형 타일·프랍·엔진 조명 분리를 요청했다. 따라서 A는 **가공 기준으로 선택**되었고 B는 비교 기록으로 남는다. [A 타일·프랍 팩](../Assets/Generated/ParallaxConcepts/service-gallery-tile-pack-v1/README.md)은 별도 시안/검증 결과이며 원본을 덮어쓰지 않았다. 가려진 면의 추정 복원·12px 패럴랙스 이동에는 이음새가 있어 별도 아트 검토가 필요하다. 사용자의 최종 픽셀 아트 승인과 본 게임 씬 반입은 아직 이루어지지 않았다.
